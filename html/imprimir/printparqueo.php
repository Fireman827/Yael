...
$idParqueoQR = $idParqueo;
$idParqueoBC = $idParqueo;
...
$printer -> feed();
title($printer, "TICKET DE PARQUEO");
if ($existe == 1)
{
  $clienteMostrar = $cliente->nombreCliente;
}
else
{
  $clienteMostrar = $cliente;
}
$hora = date( "g:i a", strtotime($datos["horaEntradaParqueo"]));
// $printer -> feed(5);
// $printer -> text("Cliente: ".$clienteMostrar);
$printer -> feed(2);
title($printer, "Hora Entrada: ".$hora);
$printer -> feed(2);
title($printer, "N placa: ".$datos["placaParqueo"]);
$printer -> feed(2);
// $printer -> text($idParqueo);
$printer -> feed(2);
$printer -> qrCode($datos["idParqueo"], Printer::QR_ECLEVEL_L, 10);
$printer -> feed(2);
// $printer -> feed(2);
$printer->setBarcodeWidth(5);
$printer -> barcode($datos["idParqueo"], Printer::BARCODE_CODE93);
