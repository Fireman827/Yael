								...
								<div class="row">
									<div class="col-lg-6 col-md-6 col-sm-6 col-xs-12">
										<div class="form-group">
											...
											<input type="text" class="form-control" id="nombreRol" name="nombreRol" placeholder="Nombre del rol" value="<?=$datosRol->nombreRol;?>">
										</div>
									</div>
									<div class="col-lg-3 col-md-3 col-sm-3 col-xs-12">
										<div class="form-group">
											<label for="rutaRol">Ruta</label>
											<select class="form-control select2" name="rutaRol" id="rutaRol">
													<?= $listaRutas=explode(',',GblTraerConfiguracion('rutaRol'));
													// echo $listaRutas;
													for ($i=0;$i<count($listaRutas) ;$i++)
													{
														list($nombreRuta,$urlRol)=explode(':',$listaRutas[$i]);
														?>
														<option value="<?=$urlRol ?>"<?=($datosRol->rutaRol==$urlRol)? 'selected':'' ?>><?= $nombreRuta ?></option>
											    	<?php
													}
													?>
											</select>
															...
															<?php if (!empty($menu->modulos)): ?>
																<?php foreach ($menu->modulos as $modulo): ?>
																	<div class="icheck-<?=GblTraerConfiguracion('colorComponentes')?> d-inline">
																		<input type="checkbox" class="permisoRol <?=($modulo->funcionModulo =="") ? "principal" : ""; ?>" id="modulo<?=$modulo->idMenuModulo?>" data-idmodulo="<?=$modulo->idMenuModulo?>" <?php echo ($modulo->existeEnRol) ? 'checked' : '';?>>
