			...
			<div class="row">
                ...
                <div class="row" style="text-align: center">
                  <div class="col-md-12" style="text-align: center;">
                    <label class="badge badge-warning" style="font-size: 15px; text-align: center">Advertencia!! Si el campo de Existencia Real se envia en cero el inventario se ajustara a esa cantidad.</label>
                  </div>
                </div>
                <div class="row">
                  ...
                  <table class="table table-hover table-sm dataTable dtr-inline" role="grid" aria-describedby="tablaTurno_info">
                    <thead>
                      <tr>
                        ...
                        <th class="col-lg-2">Existencia Inicio</th>
                        <th class="col-lg-2">Existencia Actual</th>
                      ...
                      <?php
                          if ($datosMostrar)
                          {
                            ...
                            foreach ($datosMostrar as $fila)
                            {
                              ...
                              $existenciaInicioInsumo = $fila["existenciaInicioInsumo"];
                              $idRevisionInsumo = $fila["idRevisionInsumo"];
                              $insumoActual = number_format($cantidadInsumoStock/$unidadInsumoPresentacion, 2,'.',',');
                              echo "<tr class='fila".$idCategoriaInsumo."'>";
                              echo "<td><input type='hidden' id='idInsumo' value='".$idInsumo."'><input type='hidden' id='idInsumoPresentacion' value='".$idInsumoPresentacion."'><input type='hidden' id='idRevisionInsumo' value='".$idRevisionInsumo."'>".$n."</td>";
                              echo "<td><input type='hidden' id='nombreInsumo' value='".$nombreInsumo."'>".$nombreInsumo."</td>";
                              // echo "<td><input type='hidden' id='descripcionInsumo' value='".$descripcionInsumo."'>".$descripcionInsumo."</td>";
                              echo "<td><input type='hidden' id='existenciaInicioInsumo' value='".$existenciaInicioInsumo."'>".$existenciaInicioInsumo."</td>";
                              echo "<td><input type='hidden' id='insumoActual' value='".$insumoActual."'><input type='hidden' id='unidadInsumoPresentacion' value='".$unidadInsumoPresentacion."'>".$insumoActual."</td>";
                              echo "<td><input type='hidden' id='diferencia' value='".$diferencia."'><input type='hidden' id='descripcionInsumo' value='".$descripcionInsumo."'>".$diferencia."</td>";
                              echo "<td><input type='text' class='form-control' id='existenciaReal' value=''></td>";
                              echo "</tr>";
                              $n+=1;
                            }
                          }
                ...
                <input type="hidden" name="idCorte" id="idCorte" value="<?php echo $idCorte; ?>">
								...
								<button class="btn btn-<?=GblTraerConfiguracion('colorComponentes');?> float-right" id="btnRevisarInusmoFinal"><i class="fa fa-save"></i> Guardar</button>
							</div>
					</div>
					<!-- /.card -->
				</div>
