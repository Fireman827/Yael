										...
										<div class="card-body menuPermiso">
											<?php if (!empty($menu->modulos)): ?>
												<?php foreach ($menu->modulos as $modulo): ?>
													<div class="icheck-<?=GblTraerConfiguracion('colorComponentes')?> d-inline">
														<input type="checkbox" class="permisoUsuario <?=($modulo->funcionModulo =="") ? "principal" : ""; ?>" controlador="<?=$modulo->controladorModulo?>" id="modulo<?=$modulo->idMenuModulo?>" data-idmodulo="<?=$modulo->idMenuModulo?>" <?php echo ($modulo->existeEnPermiso || $datosUsuario->adminUsuario) ? 'checked' : '';?>>
